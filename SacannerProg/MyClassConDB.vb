...
Public Class MyClassConDB

    ' Dim Cn As New SqlClient.SqlConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\QAS\QAS\data.accdb")
    Dim cmd As New SqlClient.SqlCommand ' OleDb.OleDbCommand
    Dim tbl As New DataTable
    Dim da As New SqlClient.SqlDataAdapter
    Dim sql As String
    Public conn As New SqlConnection(My.Settings.DBHRMConnectionString)
    ...
    Public Sub New()
        SqlCon = New SqlConnection(My.Settings.DBHRMConnectionString)
    End Sub
    ...
    Public Function SelectData(ByVal Stored As String, ByVal Param() As SqlParameter) As DataTable
    ...
    Public Function GetRecords(ByVal Oledb As String) As DataTable
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter(Oledb, conn)
            da.Fill(tbl)
        Catch ex As SqlException
            MsgBox(ex.Message)
        Finally
    ...
    Public Function CommandsDisplay(ByVal SqlStr As String, ByVal ColmnName As String) As SqlDataReader
        Dim command As New SqlCommand(SqlStr, conn)
    ...
    Public Sub ExecuteCommand(ByVal Sql As String, ByVal Param As SqlParameter())

        Try
            TestRunCode = True
            Dim SqlCmd As SqlCommand = New SqlCommand(Sql, Cn)
    ...
    Public Sub UserMovement(ByVal UserID As Int16, ByVal IDMove As String, ByVal MoveDescrption As String, ByVal MenuName As String)
        Try
            ...
            sql += " VALUES     ('" + UserID.ToString + "','" + IDMove + "',N'" + MoveDescrption + "',N'" + MenuName + "')" '"','" + DateAndTime.Now.ToString +
            MyClss.RunsInsertDeleteUpdateQry(sql)
        Catch ex As SqlClient.SqlException
            TestRunCode = False
            MsgBox(ex.Message)
